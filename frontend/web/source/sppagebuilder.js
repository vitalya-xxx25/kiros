...
(function ($) {
  ...
  $(document).ready(function() {
    ...
    $('#sppbSection').bind('inview', function(event, visible, visiblePartX, visiblePartY) {
      if (visible) {
        var numbers = $('.sppb-animated-number');
        $(numbers).each(function(i, elem){
            $(elem).sppbanimateNumbers($(elem).data('digit'), false, $(elem).data('duration')); 
        });
        $(this).unbind('inview');
      }
    });
